...
description: >
  ...
  Pode ser invocada pelo orquestrador ou diretamente via CLI.
---
...
`target.yaml` com `nome_completo`, `parentes` (opcional), `apelidos`
(opcional) — ou via CLI:

```bash
python skills/expand-brazilian-identity/scripts/expand_identity.py \
  --name "Ricardo Andrade Magro" \
  --alias "Rick Magro" \
  --relatives '[{"nome": "Ana Magro", "relacao": "conjuge"}]'
```
...
4. Ordem invertida: último sobrenome + primeiro nome (`Magro Ricardo`)
   — convenção de alguns formulários americanos
5. Iniciais + último sobrenome (`R. A. Magro`, `R. Magro`)
6. Sobrenome isolado (`Magro`) — busca ampla, alto ruído; marcar como tal
...
```json
{
  "target_name": "Ricardo Andrade Magro",
  "generated_at": "2026-04-30T14:00:00+00:00",
  "approved_by_human": false,
  "variations": [
    {"variation": "Ricardo Andrade Magro", "type": "full_name", "note": "..."},
    {"variation": "Ricardo Magro",         "type": "first_last", "note": "..."},
    {"variation": "Magro Ricardo",         "type": "last_first", "note": "..."},
    {"variation": "R. A. Magro",           "type": "initials_last", "note": "..."},
    {"variation": "Magro",                 "type": "surname_only", "note": "..."}
  ]
}
```
